import {
    ContainsCondition,
    FiltersCollection,
    SearchConditions,
    SortsCollection
} from "@/Components/Filters/Types/Filters";
import {Field, ListRequestConfig} from "@/Types/Request/Requests";
import {statusColors} from "@/Types/Request/Colors";
import {PaymentStatus} from "@/Types/Enums/Payments";

export const statusFilterVariants =
    [
        {
            name: 'pending',
            value: PaymentStatus.PENDING.valueOf(),
        },
        {
            name: 'success',
            value: PaymentStatus.SUCCESS.valueOf()
        },
        {
            name: 'failed',
            value: PaymentStatus.FAILED.valueOf(),
        },
    ]

export function usePaymentsList(userId: number | null = null) {
    const userContains = [];
    if (userId) {
        userContains.push(userId);
    }

    return new ListRequestConfig(
        '/api/v1/stimule/payments',
        new SearchConditions(
            ['id', 'amount', 'status', 'status_name', 'system', 'user_id', 'created_at'],
            new FiltersCollection([
                new ContainsCondition('user_id', userContains),
                new ContainsCondition('status', [], [], statusFilterVariants),
            ]),
            new SortsCollection({
                created_at: 'desc'
            }),
            1,
            100
        ),
        [
            new Field('id', 'number', true),
            new Field('amount', 'money', false, 'Amount'),
            new Field('status_name', 'string', false, 'Status', statusColors, 'status'),
            new Field('user_id', 'number', true, 'User',),
            new Field('system', 'string', false, 'System',),
            new Field('system', 'string', false, 'Provider'),
            new Field('created_at', 'date', false, 'Created at'),
        ]
    );
}
